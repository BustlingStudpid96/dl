...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        self.embedding = nn.Embedding(20, label_emb_dim)
        self.model = nn.Sequential(
            ...
            nn.BatchNorm1d(2048),
            nn.GELU(),
            nn.Linear(2048, np.prod(image_size, dtype=np.int32)),
            #  nn.Tanh(),
            nn.Sigmoid(),
        )

    def forward(self, z, labels):
        # shape of z: [batchsize, latent_dim] [64,64]
...
class Discriminator(nn.Module):
    ...
    def forward(self, image, labels):
        ...
        label_embedding = self.embedding(labels)
        combined_data = torch.cat([image.reshape(image.shape[0], -1), label_embedding], dim=-1)
        # print(combined_data.shape)  # [64, 3104]
        prob = self.model(combined_data)
        return prob


# Training
dataset_path = '../../1_17/pythonProject/datasets/'
dataset = torchvision.datasets.CIFAR10(root=dataset_path, download=True, transform=torchvision.transforms.Compose(
    [

...
class Generator(nn.Module):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.model1 = nn.Sequential(
            nn.Linear(nz, ngf * 16 * 4 * 4),  # 1024*4*4
            nn.BatchNorm1d(ngf * 16 * 4 * 4),
        )  # [64,1024,4,4]
        self.model2 = nn.Sequential(
            # 第一个转置卷积层 z:[64,512,4,4]
            # nn.ConvTranspose2d(in_channels=512,
            #                    out_channels=ngf * 16,  # 1024
            #                    kernel_size=4, stride=1, padding=0),
            # nn.BatchNorm2d(ngf * 16),
            # nn.ReLU(inplace=True),
            # 第二个转置卷积层 [64,1024,8,8]
    ...
    def forward(self, z):  # z : [64*100]
        # z = z.reshape(z.size(0), z.size(1), 1, 1)
        z = self.model1(z)
        # print(z.shape)  # torch.Size([64, 2048])
        z = z.reshape(z.size(0), ngf * 16, 4, 4)  # torch.Size([64, 512, 2, 2])
        # print(z.shape)
        z = self.model2(z)
        # print(z.shape)  #torch.Size([64, 3, 160, 160])
        # 裁剪中心96*96作为生成图片
        start_h = (z.size(2) - 96) // 2
        start_w = (z.size(3) - 96) // 2
        cropped_z = z[:, :, start_h:start_h + 96, start_w:start_w + 96]
...
# Training
dataset_path = 'datas/f/'
trans = transforms.Compose([
    # transforms.Resize((64, 64)),  # 根据需要调整图片大小
    transforms.ToTensor(),  # 将图片转换为Tensor
    transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5]),  # 归一化
])
face_dataset = torchvision.datasets.ImageFolder(root=dataset_path, transform=trans)
dataloader = torch.utils.data.DataLoader(face_dataset, batch_size=batch_size, shuffle=True, drop_last=True)
generator = Generator().to(device)
discriminator = Discriminator().to(device)

# g_optimizer = torch.optim.Adam(generator.parameters(), lr=0.0003, betas=(0.4, 0.8), weight_decay=0.0001)
# d_optimizer = torch.optim.Adam(discriminator.parameters(), lr=0.0003, betas=(0.4, 0.8), weight_decay=0.0001)
g_optimizer = torch.optim.RMSprop(generator.parameters(), lr=0.0003, weight_decay=0.0001)
d_optimizer = torch.optim.RMSprop(discriminator.parameters(), lr=0.0003, weight_decay=0.0001)
...
for epoch in range(num_epochs):
    for i, (gt_images, labels) in enumerate(dataloader):
        ...
        # print(gt_images.shape, labels.shape)

        z = torch.randn(batch_size, nz, device=device)  # [64,100]
        ...
        if g_loss < best_g_loss:
            best_g_loss = g_loss
            g_state['state_dict'] = g_optimizer.state_dict()
            g_state['g_loss'] = g_loss
            torch.save(g_state, 'g_states.pth')
        g_loss.backward()
        g_optimizer.step()
        if i % 2 == 0:
            ...
            if d_loss < best_d_loss:
                best_d_loss = d_loss
                d_state['state_dict'] = d_optimizer.state_dict()
                d_state['d_loss'] = d_loss
                torch.save(d_state,'d_states.pth')
            d_loss.backward()
            d_optimizer.step()
        ...
        if i % 400 == 0:
            ...
            torchvision.utils.save_image(image, f"./gimgs/image_{len(dataloader) * epoch + i}.png", nrow=4)
# generator = Generator()
